...
class TestIMDbSoup(unittest.TestCase):
    ...
    def test_extract_genre(self):
        self.assertEqual(IMDbSoup(self.test_id_list[0]).extract_genre(), 'Documentary, Short')
        self.assertEqual(IMDbSoup(self.test_id_list[1]).extract_genre(), 'Game-Show, Music, Reality-TV')
        self.assertEqual(IMDbSoup(self.test_id_list[2]).extract_genre(), 'Comedy')
        self.assertEqual(IMDbSoup(self.test_id_list[12]).extract_genre(), None)

    def test_extract_time(self):
        self.assertEqual(IMDbSoup(self.test_id_list[0]).extract_runtime(), 1)
        self.assertEqual(IMDbSoup(self.test_id_list[1]).extract_runtime(), 60)
        self.assertEqual(IMDbSoup(self.test_id_list[2]).extract_runtime(), 30)
